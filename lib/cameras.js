...
const RESTART_DELAY_MS = 5000; // 首次重試延遲
const RESTART_DELAY_MAX_MS = 60000; // 連續失敗時最長等 60 秒，避免狂重試洗資源
const STABLE_RUN_MS = 30000; // 撐超過這麼久才算「有成功過」，重置退避
const GRACE_RETRIES = 3; // 前幾次仍用固定 5 秒（剛重開時相機還沒準備好很正常）
...
export class CameraManager {
  constructor(recording) {
    ...
    this.fastFails = new Map(); // id -> 連續「一啟動就掛」的次數（指數退避用）
  ...
  update(id, patch) {
    ...
    this.fastFails.delete(id); // 改過設定就重置退避，讓修好的網址能馬上重試
  ...
  _spawn(camera) {
    ...
    const entry = { proc, camera, status: 'starting', spawnedAt: Date.now() };
    ...
    proc.on('exit', (code, signal) => {
      ...
      // 退避策略：撐夠久（曾正常運作）就歸零；一啟動就掛則累計次數。
      // 前幾次仍用固定 5 秒（剛重開時相機還沒準備好很正常，要讓它們快點連回來），
      // 連續失敗超過 GRACE_RETRIES 次才開始加倍等待，最多 60 秒，
      // 這樣「暫時連不上」的相機恢復快，「網址錯／離線」的才會被拉長、不洗資源。
      const ranMs = Date.now() - entry.spawnedAt;
      let fails;
      if (ranMs >= STABLE_RUN_MS) {
        fails = 0;
      } else {
        fails = (this.fastFails.get(camera.id) || 0) + 1;
      }
      this.fastFails.set(camera.id, fails);
      const over = Math.max(0, fails - GRACE_RETRIES);
      const delay = Math.min(
        RESTART_DELAY_MS * Math.pow(2, over),
        RESTART_DELAY_MAX_MS
      );

      console.warn(
        `⚠️  ${camera.name} 中斷 (code=${code})，${Math.round(delay / 1000)} 秒後重試`
      );
      setTimeout(() => {
