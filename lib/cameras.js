...
export class CameraManager {
  constructor(recording) {
    ...
    this.retryMs = new Map(); // id -> 目前的重試延遲（指數退避用）
  ...
  update(id, patch) {
    ...
    this.retryMs.delete(id); // 改過設定就重置退避，讓修好的網址能馬上重試
  ...
  _spawn(camera) {
    ...
    proc.on('exit', (code, signal) => {
      ...
      // 指數退避：撐夠久（曾正常運作）就從頭算；一啟動就掛（例如網址錯、
      // 相機離線）則每次把等待時間加倍，最多 60 秒，避免每 5 秒狂重試洗資源。
      const ranMs = Date.now() - entry.spawnedAt;
      let delay;
      if (ranMs >= STABLE_RUN_MS) {
        delay = RESTART_DELAY_MS;
      } else {
        const prev = this.retryMs.get(camera.id) || RESTART_DELAY_MS;
        delay = Math.min(prev * 2, RESTART_DELAY_MAX_MS);
      }
      this.retryMs.set(camera.id, delay);
